 ...
 * (folder Host-Minecraft-final/data/) supaya begitu admin konfirmasi
 * pembayaran, paket/VIP user otomatis aktif di panel BlockHost.
 *
 * Mendukung 2 jenis pembelian lewat form yang sama:
 *  - "paket" -> update tier + tierExpiry akun di data/users.json
 *  - "vip"   -> update tier VIP pemain di data/vip.json (by nama in-game)
 *
 * Cara kerja:
 ...
 *  - Saat dikonfirmasi             -> data terkait diupdate LANGSUNG di
 *                                     Host-Minecraft-final/data/ (file yang sama dipakai panel)
 *
...
// Folder panel BlockHost ada di sebelah folder payment-confirm ini.
// Kalau lokasi folder panelmu beda, ubah baris ini.
const BLOCKHOST_DIR = path.join(__dirname, '..', 'Host-Minecraft-final');
const BLOCKHOST_USERS_PATH = path.join(BLOCKHOST_DIR, 'data', 'users.json');
const BLOCKHOST_VIP_PATH = path.join(BLOCKHOST_DIR, 'data', 'vip.json');

const DATA_DIR = path.join(__dirname, 'data');
const PAYMENTS_PATH = path.join(DATA_DIR, 'payments.json');

// GANTI password admin ini sebelum dipakai sungguhan!
const ADMIN_KEY = process.env.ADMIN_KEY || 'TAMAEL999';
...
// Tier VIP pemain (harus sama persis dengan id/urutan VIP_TIERS di server.js
// panel BlockHost — 1 = VIP I, 2 = VIP II, 3 = VIP III). Harga di sini cuma
// dipakai untuk ditampilkan di form, edit sesuai keinginan.
const VIP_TIER_IDS = { 'VIP I': 1, 'VIP II': 2, 'VIP III': 3 };
// ==========================
...
function activateTierForUser(email, tier, price) {
  ...
  if (!users) {
    return { ok: false, error: 'Tidak menemukan Host-Minecraft-final/data/users.json. Pastikan folder panel ada di ../Host-Minecraft-final dan user sudah pernah daftar akun lewat panel (supaya file itu terbuat).' };
  }
...
// ====== Aktifkan VIP pemain di BlockHost (file data/vip.json yang sama dipakai panel) ======
function activateVipForPlayer(playerName, vipTierLabel) {
  const name = String(playerName || '').trim();
  if (!name) return { ok: false, error: 'Nama akun/pemain Minecraft wajib diisi.' };
  const tierId = VIP_TIER_IDS[vipTierLabel];
  if (!tierId) return { ok: false, error: `Tier VIP "${vipTierLabel}" tidak dikenal.` };

  // Beda dengan users.json (butuh akun terdaftar dulu), vip.json boleh belum
  // ada sama sekali — kalau file/foldernya belum ada, buat baru saja karena
  // entri VIP cukup diidentifikasi dari nama pemain, tanpa perlu akun panel.
  const vipDataDir = path.dirname(BLOCKHOST_VIP_PATH);
  if (!fs.existsSync(vipDataDir)) {
    return { ok: false, error: 'Tidak menemukan folder Host-Minecraft-final/data. Pastikan folder panel ada di ../Host-Minecraft-final dan panel pernah dijalankan minimal sekali (supaya folder itu terbuat).' };
  }
  const vip = loadJSON(BLOCKHOST_VIP_PATH, {});
  const key = name.toLowerCase();
  const prevEntry = vip[key];
  // Kalau sebelumnya VIP III (auto-op) dan pembelian baru bukan VIP III,
  // status autoOpped tidak dibawa — panel yang akan lepas op-nya otomatis
  // saat admin/panel memproses perubahan tier ini.
  ...
  saveJSON(BLOCKHOST_VIP_PATH, vip);
  return { ok: true, vip: vip[key] };
}
...
const server = http.createServer((req, res) => {
  ...
  // ---- CORS: izinkan panel BlockHost (port beda) memanggil API ini dari browser ----
  ...
  if (p === '/api/payment/submit' && req.method === 'POST') {
    return readBody(req, (err, body) => {
      ...
      const type = String(body.type || 'paket').trim() === 'vip' ? 'vip' : 'paket';
      ...
      if (type === 'vip') {
        if (!VIP_TIER_IDS[tier]) return sendJSON(res, 200, { ok: false, error: 'Pilih tier VIP yang valid.' });
        if (!player) return sendJSON(res, 200, { ok: false, error: 'Nama akun Minecraft (in-game) wajib diisi untuk pembelian VIP.' });
      } else {
        if (!['Batu', 'Besi', 'Emas', 'Berlian'].includes(tier)) {
          return sendJSON(res, 200, { ok: false, error: 'Pilih paket yang valid.' });
        }
      }

      const entry = {
        id: 'pay_' + Date.now() + '_' + crypto.randomBytes(3).toString('hex'),
        type, email, name, tier, price, reference, note, player,
  ...
  if (p === '/api/payment/admin/confirm' && req.method === 'POST') {
    return readBody(req, (err, body) => {
      ...
      const result = entry.type === 'vip'
        ? activateVipForPlayer(entry.player, entry.tier)
        : activateTierForUser(entry.email, entry.tier, entry.price);
